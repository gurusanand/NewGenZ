"""
Zurich Edge AI Insurance Platform - Helper Functions Module
...
def generate_travel_insurance_response(user_input):
    ...
    if "japan" in user_input_lower or "tokyo" in user_input_lower:
        return """Great choice! Japan is a wonderful destination. For travel to Japan, I recommend our Premium coverage which includes:
        
        • Medical coverage up to $100,000
        • Trip cancellation/interruption protection
        • Emergency evacuation coverage
        • Coverage for natural disasters (earthquakes, typhoons)
        • 24/7 multilingual assistance
        
        Based on current weather data, there's a low risk period coming up. Would you like me to generate a personalized quote?"""
    
    elif "europe" in user_input_lower:
        return """Europe is fantastic! For European travel, our Standard coverage typically works well:
        ...
        The weather forecast shows stable conditions across most of Europe. What specific countries are you planning to visit?"""
    
    elif "quote" in user_input_lower or "price" in user_input_lower:
        return """I'd be happy to provide a quote! I'll need a few details:
        ...
        You can use the Quick Quote Form below, or continue chatting with me for a personalized recommendation."""
    
    elif "weather" in user_input_lower:
        return """Weather is a crucial factor in travel insurance! Our AI analyzes real-time weather data and forecasts to:
        
        • Adjust premiums based on weather risks
        • Provide early warnings for severe weather
        • Offer enhanced coverage for weather-related disruptions
        
        Where are you planning to travel? I can check the current weather risk assessment."""
    
    else:
        return """Hello! I'm here to help you find the perfect travel insurance for your trip. I can assist with:
        
        • Personalized quote generation
        • Coverage recommendations based on destination
...
def generate_travel_quote(destination, departure_date, return_date, travelers, coverage_type, trip_cost):
    """Generate a travel insurance quote"""
    ...
    # Weather risk adjustment (mock)
    weather_risk = get_weather_risk_factor(destination)
    
    # Calculate base premium
    base_premium = trip_cost * base_rates[coverage_type]
    
    # Apply adjustments
    weather_adjustment = base_premium * weather_risk
    duration_adjustment = max(0, (duration - 7) * 5)  # $5 per day over 7 days
    traveler_adjustment = (travelers - 1) * base_premium * 0.8  # 80% for additional travelers
    
    total_premium = base_premium + weather_adjustment + duration_adjustment + traveler_adjustment
    ...
    quote = {
        ...
        'duration_adjustment': round(duration_adjustment, 2),
        'traveler_adjustment': round(traveler_adjustment, 2),
...
def get_weather_risk_factor(destination):
    """Get weather risk factor for destination (mock)"""
    risk_factors = {
        'japan': 0.15,
        'tokyo': 0.15,
        'florida': 0.25,
    ...
    for location, risk in risk_factors.items():
        if location in destination_lower:
            return risk
    
    return 0.10  # Default risk factor
